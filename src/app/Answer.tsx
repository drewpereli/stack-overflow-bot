...
export default function Answer({title, content}: {title: string; content: string}) {
  ...
  const [status, setStatus] = useState<'initializing'|'ready'|'done'>('initializing');
  ...
  useEffect(() => {
    if (!isLoading && status === 'ready') {
      ...
      setResponses(prev => [...prev, { id: crypto.randomUUID(), content: '' }]);
